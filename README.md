...
The repository's coding standards are maintained in the [Copilot instructions](.github/copilot-instructions.md) and the focused [Astro](.github/instructions/astro.instructions.md), [data-layer](.github/instructions/drizzle.instructions.md), and [UI](.github/instructions/ui.instructions.md) instruction files. In particular:

- Comments explain intent, constraints, and decisions rather than restating code, and must be kept current.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc for their purpose, parameters, return values, and injectable database argument.
- Reusable `.astro` components document their public `Props` interface.
- TypeScript follows the repository's explicit typing and formatting conventions and must pass ESLint.
